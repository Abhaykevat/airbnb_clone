...
class PostingModel
{
  ...
  getFirstImageFromStorage() async
  {
    if(displayImages!.isNotEmpty)
      ...
      final imageData = await FirebaseStorage.instance.ref()
          ...
          .child(imageNames!.first).getData(1024*1024);
      displayImages!.add(MemoryImage(imageData!));
      return displayImages!.first;
  }
  ...
  String getBedroomText()
  {
    ...
    {
      text=text + beds!["large"].toString()+"large";
    }
    return text;
  ...
  String getBathroomText()
  {
    ...
    if(this.beds!["full"] != 0)
    {
      text=text + beds!["full"].toString()+"full";
    }
    if(this.beds!["half"] !=0)
    {
      text=text + beds!["half"].toString()+"half";
    }
  ...
  Future<void> makeNewBooking(List<DateTime> dates,context,hostID) async
  {
    ...
    DocumentReference reference=await FirebaseFirestore.instance.collection('postings').doc(id).collection('bookings').add(bookingData);
    BookingModel newBooking=BookingModel();

    newBooking.createBooking(this,AppConstants.currentUser.createUserFromContact(),
        dates);
